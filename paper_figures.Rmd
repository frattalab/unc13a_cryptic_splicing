...
## Systematic differences in  (read length) between cases and controls

```{r sequences and case/controls}
# library size 
#llumina HiSeq 2500 (125 bp paired end) or an Illumina NovaSeq (100 bp paired end). 
case_control_sequencing <- clean_data_table %>%
...
quick_chiq = function(tbl){
  
    if(length(unique(tbl$sequencing_platform)) < 2){
      return(NA)
    }
    comparison_table = table(disease = tbl$disease_group2, 
                             sequencing_platform = tbl$sequencing_platform)
    print(comparison_table)

    p_value = chisq.test(comparison_table) %>%  broom::tidy() %>% pull(p.value)
    browser()
    print(p_value)
...
case_control_sequencing %>% 
  group_by(tissue_clean) %>% 
  nest()  %>% 
  mutate(chiq_pvalue = map_dbl(data, quick_chiq)) 
...
plot_list = list()
for(t in 1:length(lib_comps)){
    if(grepl("Cord",lib_comps[t]) | lib_comps[t] == "Motor_Cortex"){
        groups = c("Control","ALS-TDP","ALS \n non-TDP")
    }else if(lib_comps[t] %in% c("Frontal_Cortex","Temporal_Cortex")){
        groups = c("Control","FTLD-TDP","FTLD \n non-TDP")
    }else{
...
comps = only_short_plotter %>% pull(detection_name) %>%
  unique() %>%
  combn(.,2,simplify = F)
