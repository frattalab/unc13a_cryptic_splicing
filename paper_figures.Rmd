...
<!-- # iPSC WTC11 parent line genotype -->

<!-- ```{r confirm_no_als_ftd_wtc11 mutations} -->
<!-- library(vcf) -->
<!-- wtc11_vcf = "/Users/annaleigh/Downloads/3546dc62_AH77TTBBXX_DS-229105_GCCAAT_recalibrated.vcf.gz" -->
<!-- vcf <- read.vcfR( wtc11_vcf, verbose = FALSE ) -->
<!-- ``` -->
...
clean_data_table %>%
  ...
  facet_wrap(~ tissue + disease_group2) + 
  ggpubr::stat_compare_means(label = "p.format", label.x.npc = "center", method = "wilcox" ) +
  labs(y = "log10 library size", x = "UNC13A CE detected") +
...
clean_data_table %>%
  ...
  mutate(inclusion_reads = UNC13A_3prime_leaf + UNC13A_5prime_1_leaf) %>% 
  mutate(unc13a_detected = ifelse(inclusion_reads > 0, yes = "+", no = "-") ) %>% 
  ggplot(aes(x = unc13a_detected,y = UNC13A_TPM )) + 
  geom_point( colour = "firebrick", size = 0.5, alpha = 0.9, position = position_jitter(width = 0.2, height = 0) ) + 
  geom_boxplot(fill = NA, colour = "black", outlier.colour = NA) +
  labs(title = "UNC13A TPM and UNC13A CE detection ",subtitle = "Wilcox test") + 
  facet_wrap(~ tissue + disease_group2, scales = 'free_y') + 
  ggpubr::stat_compare_means(label = "p.format", label.x.npc = "center", method = "wilcox" ) +
  labs(y = "UNC13A expression (TPM)", x = "UNC13A CE detection") +
  theme(axis.text.x = element_text(size = 15)) +
  ggpubr::theme_pubr()
...
clean_data_table %>%
  ...
  geom_point( colour = "firebrick", size = 0.5, alpha = 0.9, position = position_jitter(width = 0.2, height = 0) ) + 
   geom_boxplot(fill = NA, colour = "black", outlier.colour = NA) +
  labs(title = "Sample RIN and UNC13A CE detection ",subtitle = "Wilcox test") + 
  facet_wrap(~disease_group2 + tissue) + 
  ggpubr::stat_compare_means(label = "p.format", label.x.npc = "center" ) +
  labs(y = "RIN", x = "UNC13A CE detection") +
  theme(axis.text.x = element_text(size = 15)) +
  ggpubr::theme_pubr()
...
cell_types = fread("/Users/annaleigh/Documents/GitHub/unc13a_cryptic_splicing/data/MuSiC_residual_results.tsv") 
