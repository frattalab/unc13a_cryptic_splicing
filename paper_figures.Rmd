...
<<<<<<< Updated upstream
=======
>>>>>>> 726646a264376675e20bc66e2b4658c1aa88cd71
```
...
=======
    ...
    geom_text(data = text_table[gene_name == "UNC13A"],aes(x = source, y= 1.2,label = padj_plot),size = 8) + 
    geom_text(data = text_table[gene_name == "UNC13A"],aes(x = source, y= 1.35,label = log2FoldChange_plot),size = 8) 
>>>>>>> 726646a264376675e20bc66e2b4658c1aa88cd71
...
=======
    scale_y_continuous(labels = scales::percent,expand = expansion(mult = c(0, .1))) +
    geom_text(data = text_table[gene_name == "UNC13B"],aes(x = source, y= 1.2,label = padj_plot),size = 8) + 
    geom_text(data = text_table[gene_name == "UNC13B"],aes(x = source, y= 1.35,label = log2FoldChange_plot),size = 8) 
>>>>>>> 726646a264376675e20bc66e2b4658c1aa88cd71
...
## Scatterplot UNC13B-IR and UNC13A Crptic
...
for_bar_plot_sy <- read_csv(file.path(here::here(),"data","results_17022021_sy5y.csv"))

for_bar_plot_sy <- data.frame(for_bar_plot_sy)
for_bar_plot_sy$condition <- as.factor(for_bar_plot_sy$condition)
for_bar_plot_sy$condition <- factor(for_bar_plot_sy$condition,levels(for_bar_plot_sy$condition)[c(2,1)])
...
for_bar_plot_dz <- read_csv(file.path(here::here(),"data","results_17022021_dz.csv"))

for_bar_plot_dz <- data.frame(for_bar_plot_dz)
for_bar_plot_dz$condition <- as.factor(for_bar_plot_dz$condition)
for_bar_plot_dz$condition <- factor(for_bar_plot_dz$condition,levels(for_bar_plot_dz$condition)[c(2,1)])
    
nmd_plot2 = ggbarplot(for_bar_plot_dz,
                        x = 'gene',
                        add = c("mean_se","jitter"),
                        y = 'value',
                        fill = 'condition',
...
clean_data_table %>% 
  ...
  theme(text = element_text(size = 24)) +
  stat_compare_means() +
  stat_compare_means(comparisons = rs12973192_comparisons,label = "p.signif") + 
...
clean_data_table %>% 
  ...
  theme(text = element_text(size = 24)) +
  stat_compare_means() +
  stat_compare_means(comparisons = rs12608932_comparisons,label = "p.signif") + 
  scale_y_continuous(trans='log10',labels = scales::percent)
...
## UNC13A psi by genotype - rs12608932
```{r}
