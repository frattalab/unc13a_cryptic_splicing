...
## Effect of seqeuncer (read length) size on CE discovery 
...
unc13a_data_fisher = unc13a_sequencing_platform %>% 
  ...
  split(.,list(unique(unc13a_sequencing_platform$tissue_clean), unique(unc13a_sequencing_platform$disease_group2))) %>% 
  purrr::map_df( quick_fisher, .id = "tissue_clean" ) %>% 
  melt() %>% 
  select(variable,value) %>% 
  separate(variable,sep = "\\.", into = c("tissue_clean","disease_group2"))
...
unc13a_sequencing_platform_pct %>%
  ...
  labs(title = "UNC13A CE detection and sequencing platform", y = "UNC13A CE detected") + 
  facet_wrap(~disease_group2 + tissue_clean,scales = 'free_x') +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  geom_text(aes(x = 1.5, y = 1.05, label = paste0("p = ", signif(value,digits = 2) ) )) +
  theme(legend.text = element_text(size = 15))
 # scale_y_continuous(expand = c(0,0) ) 
```
## Effect of library size on CE discovery 
...
clean_data_table %>%
  ...
  ggplot(aes(x = disease_group2,y = log10(library_size), fill = unc13a_detected)) + 
  ...
  labs(title = "Library size and UNC13A CE detection ",subtitle = "Wilcox test") + 
...
## Effect of UNC13A TPM onCE discovery 

```{r unc detection by UNC13A tpm }
# library size 
...
clean_data_table %>%
  ...
  mutate(unc13a_detected = ifelse(inclusion_reads > 0, yes = "+", no = "-")) %>% 
  mutate(disease_group2 = fct_relevel(disease_group2, "Control")) %>% 
  ggplot(aes(x = disease_group2,y = UNC13A_TPM, fill = unc13a_detected)) + 
  geom_point(colour = "firebrick", size = 0.5, alpha = 0.9, position = position_jitter(width = 0.2, height = 0) ) + 
  geom_boxplot(colour = "black", outlier.colour = NA) +
  labs(title = "UNC13A TPM and UNC13A CE detection ",subtitle = "Wilcox test") + 
  facet_wrap(~ tissue, scales = 'free') + 
  ggpubr::stat_compare_means(label = "p.format", label.x.npc = "center", method = "wilcox" ) +
  labs(y = "UNC13A TPM", fill= "UNC13A CE detected", x = element_blank()) +
  theme(axis.text.x = element_text(size = 15)) + 
  ggpubr::theme_pubr()

```

## Effect of RIN and UNC13A discovery
...
clean_data_table %>%
  ...
  mutate(unc13a_detected = ifelse(inclusion_reads > 0, yes = "+", no = "-") ) %>% 
    mutate(disease_group2 = fct_relevel(disease_group2, "Control")) %>% 
  ggplot(aes(x = disease_group2,y = rin, fill = unc13a_detected)) + 
  geom_point( colour = "firebrick", size = 0.5, alpha = 0.9, position = position_jitter(width = 0.2, height = 0) ) + 
   geom_boxplot(colour = "black", outlier.colour = NA) +
  labs(title = "Sample RIN and UNC13A CE detection ") + 
  facet_wrap(~ tissue, scales = 'free_x') + 
  ggpubr::stat_compare_means(label = "p.format", label.x.npc = "center") +
  labs(y = "RIN", x = element_blank(), fill = "UNC13A CE detection") +
  theme(axis.text.x = element_text(size = 15)) +
  ggpubr::theme_pubr()
...
cell_types %>% 
  ...
  mutate(inclusion_reads = UNC13A_3prime_leaf + UNC13A_5prime_1_leaf) %>% 
  mutate(unc13a_detected = ifelse(inclusion_reads > 0, yes = "+", no = "-")) %>% 
  add_count(disease_group2,tissue_clean) %>% 
  mutate(n = n / 5) %>% 
  mutate(plot_name = glue::glue("{disease_group2} \n {n}")) %>% 
  ...
  facet_wrap(~plot_name+tissue,scales = 'free') +
  ggpubr::stat_compare_means(label = "p.signif") +
  ggpubr::stat_compare_means(label = "p.format",vjust = 1.2) +
  labs(title = "Cell type decomposition and UNC13A CE detection ",subtitle = "Wilcox test") + 
  ggpubr::theme_pubr() 
...
als_ftd_genomics = c("APP", "CHMP2B", "CSF1R", "DNAJC5","UNC13A",
                     "DNMT1", "EPM2A", "GRN", "ITM2B",
                     "MAPT", "NHLRC1", "NOTCH3", "PRNP", 
                     "PSEN1", "PSEN2", "TARDBP", "TBK1", 
                     "TYROBP", "UBQLN2", "VCP", "ATN1", 
                     "ATXN10", "ATXN1", "ATXN2", "C9orf72", 
                     "HTT", "JPH3", "NOP56", "TBP", "ALS2", 
                     "ANG", "DCTN1", "FIG4", "FUS", "HNRNPA1", 
                     "OPTN", "PFN1", "SETX", "SIGMAR1", "SLC52A2", 
                     "SLC52A3", "SOD1", "VAPB", "AR")

ipsc_de = as.data.table(ipsc_de)
ipsc_splicing = as.data.table(ipsc_splicing)

ipsc_de[gene_name %in% als_ftd_risk_genes][order(log2FoldChange)][,.(log2FoldChange,padj,gene_name,gene_expresion)] %>% mutate(has_cryptic = gene_name %in% ipsc_splicing[cryptic_junction == T,gene_name]) %>% unique() 
```
